# FAQ — UK MHRA Yellow Card — COVID vaccines
...
filter: covid_vaccine (447,845). Filter `gn_spare='covid_vaccine'` for the COVID-vaccine
...
**Why might row-count not equal report-count?** ROWS ARE INDIVIDUAL REACTIONS, not patient reports — one report can produce several rows. Count rows as reactions; there is no patient key.
